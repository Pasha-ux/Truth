...
function handleAnswer(userAnswer) {
  ...
  const otherBtn = userAnswer ? btnMyth : btnTruth;
  ...
  setTimeout(function () {
    ...
    btnNext.textContent = currentIndex < questions.length - 1 ? "Далi" : "Результат";
  }, 600);
}
...
function addQuestion() {
  ...
  if (!text) {
    newQuestionTextEl.focus();
    return;
  }

  if (!explanation) {
    newQuestionExplanationEl.focus();
    return;
  }
